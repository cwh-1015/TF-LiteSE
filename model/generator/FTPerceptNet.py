...
from .dpr_layer import CustomLayerNorm, RNN2, DPR
...
class FTMix(nn.Module):
    ...
    def forward(self, x):             # (B,C,T,257)
        x = self.freq_conv(x)         # 258→129 (奇数→奇数)
        x = self.time_conv(x)
        return x
...
class FTPerceptNet(nn.Module):
    def __init__(self, num_channels=16, out_channel=2, n_blocks=2, n_fft=512, hop_length=256, compress_factor=0.3):
        ...
        self.proc = nn.Sequential(*[DPR(emb_dim=num_channels, hidden_dim=num_channels*3//2, n_freqs=64, dropout_p=0.1,) for _ in range(n_blocks)])
    ...
    def forward(self, src, tgt=None):               # (B,4,T,257)
