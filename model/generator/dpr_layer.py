...
def pad_to_multiple(x: torch.Tensor, multiple: int, dim: int = 1):
    """
    将张量 x 在指定维度 dim 上 pad，长度扩展到能被 `multiple` 整除。
    返回 (x_padded, pad_len)，其中 pad_len 是在 tail 上填充的元素数。
    """
    length = x.size(dim)
    pad_len = (-length) % multiple  # 如果 length%multiple==0 则 pad_len=0
    if pad_len == 0:
        return x, 0
    pad_sizes = [0] * (2 * x.dim())
    # 在 dim 这一维的尾部 pad pad_len 个元素：
    # pad_sizes 从最后一对 (right, left) 开始，[..., pad_right_dimN, pad_left_dimN, pad_right_dimN-1, pad_left_dimN-1, ...]
    # 第一次出现就是对最后一维；要对第 dim 维，则 index = 2*(x.dim()-dim)-2
...
class BetaMish(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x * torch.tanh(beta * F.softplus(x))
...
class DualPathRNN(nn.Module):
    def __init__(
        ...
        self.intra_rnn_attn = RNN2(emb_dim, hidden_dim // 2, 2, dropout_p, bidirectional=True)

        self.inter_norm = nn.LayerNorm((n_freqs, emb_dim))
        self.inter_rnn_attn = RNN2(emb_dim, hidden_dim, 4, dropout_p, bidirectional=False)
