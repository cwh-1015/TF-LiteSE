...
class PesqLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        self.intelli_lambda = intelli_lambda
    ...
    def raw(
        ...
        # 1) 计算每帧每条语道的包络相关系数 corr ∈ [–1,1]
        corr = torch.sum(deg_loud * ref_loud, dim=1) \
            / (torch.sqrt(torch.sum(deg_loud**2, dim=1) * torch.sum(ref_loud**2, dim=1)) + 1e-8)
        # 2) 转成 [batch, 1] 权重，并映射到 [1, 1+λ]
        intelli_w = 1.0 + self.intelli_lambda * (1.0 - corr).unsqueeze(1)
        intelli_w = intelli_w.clamp(min=1.0, max=1.1)  
        # 3) 给对称/不对称扰动加权
        # disturbance = disturbance * intelli_w
